...
$routeKeys = $argv[1] ? array_slice($argv, 1) : ['info', 'error', 'warn'];
...
//参数：队列名，交换机名，路由键名
foreach ($routeKeys as $k => $v) {
    $channel->queue_bind($queueName, 'mq_sms_send_ex6', $v);
}
...
$channel->basic_consume($queueName, 'consumer1', false, true, false, false, $callback);
